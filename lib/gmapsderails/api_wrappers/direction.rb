module Gmapsderails
  class Direction
    ...
    def raise_net_status
      raise Gmapsderails::DirectionNetStatus, "The request sent to google was invalid (not http success): #{base_request}.\nResponse was: #{response}"
    end
    
    def raise_query_error
      raise raise Gmapsderails::DirectionStatus, "The query you passed seems invalid, status was: #{parsed_response["status"]}.\nRequest was: #{base_request}"
    end
    
    def raise_invalid
      raise Gmapsderails::DirectionInvalidQuery, "Origin and destination must be provided in a hash as first argument"
    end
