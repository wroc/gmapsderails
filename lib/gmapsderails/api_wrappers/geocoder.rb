module Gmapsderails
  class Geocoder
    ...
    def initialize(address, options = {})
      raise Gmapsderails::GeocodeInvalidQuery, "You must provide an address" if address.empty?
    ...
    def raise_net_status
      raise Gmapsderails::GeocodeNetStatus, "The request sent to google was invalid (not http success): #{base_request}.\nResponse was: #{response}"
    end
    
    def raise_query_error
      raise Gmapsderails::GeocodeStatus, "The address you passed seems invalid, status was: #{parsed_response["status"]}.\nRequest was: #{base_request}"
    end
