module Gmapsderails
  class Places
    ...
    def raise_invalid
      raise Gmapsderails::PlacesInvalidQuery, "You must provide at least a lat/lon for a Google places query"
    end
    ...
    def raise_net_status
      raise Gmapsderails::PlacesNetStatus, "The request sent to google was invalid (not http success): #{base_request}.\nResponse was: #{response}"
    end
    
    def raise_query_error
      raise Gmapsderails::PlacesStatus, "The address you passed seems invalid, status was: #{parsed_response["status"]}.\nRequest was: #{base_request}"
    end
